 ...
 * Uses PapaParse's download option for potentially better memory handling with large files.
...
export const loadTorinoTrafficData = async (): Promise<TorinoTrafficDataRow[]> => {
  const filePath = '/sa.txt';
  return new Promise((resolve, reject) => {
    Papa.parse(filePath, { // Pass the URL directly
      download: true, // Enable PapaParse to fetch the file
      header: true,
      skipEmptyLines: true,
