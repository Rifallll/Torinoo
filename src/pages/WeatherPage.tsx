...
import { ArrowLeft, CloudSun, Thermometer, Wind, Droplet, CloudRain, Sun } from 'lucide-react';
...
const WeatherPage: React.FC = () => {
  ...
  const getWeatherIcon = (iconCode: string) => {
    ...
    if (iconCode.includes('02') || iconCode.includes('03') || iconCode.includes('04')) return <CloudSun className="h-6 w-6 text-yellow-500" />;
    if (iconCode.includes('09') || iconCode.includes('10')) return <CloudRain className="h-6 w-6 text-blue-500" />;
    if (iconCode.includes('11')) return <CloudRain className="h-6 w-6 text-red-500" />;
    if (iconCode.includes('13')) return <CloudSun className="h-6 w-6 text-gray-500" />;
    if (iconCode.includes('50')) return <CloudSun className="h-6 w-6 text-gray-500" />;
      ...
      <main className="flex-1 flex justify-center items-start">
        <Card className="max-w-2xl w-full bg-white dark:bg-gray-800 shadow-lg">
          ...
          <CardContent className="space-y-4 text-gray-700 dark:text-gray-300">
            ...
            <p className="text-lg text-gray-600 dark:text-gray-400">{data.name}, Italy</p>
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="flex items-center">
                ...
                <span>Wind Speed: {Math.round(data.wind.speed * 3.6)} km/h</span>
              </div>
            </div>
