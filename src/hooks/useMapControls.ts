...
export const useMapControls = ({ map, layerGroups, tomtomTrafficFlowLayer, torinoCenter, defaultZoom }: MapControlsProps) => {
  ...
  useEffect(() => {
    ...
    // Prevent adding controls multiple times if the effect re-runs but map instance is the same
    if (geocoderRef.current) {
      return;
    }

    // Use map.whenReady() to ensure the map's internal structure is fully initialized
    map.whenReady(() => {
      ...
      fullscreenControlRef.current = fullscreenControl; // Store reference
      ...
      layerControlRef.current = layerControl; // Store reference
      ...
      resetViewControlRef.current = resetViewControl; // Store reference
    });

    // Cleanup function for the useEffect
    return () => {
      if (map) {
        // Remove controls using their stored references
